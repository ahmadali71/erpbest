...
export const POSTerminalView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full gap-4 overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white rounded-3xl ring-1 ring-slate-200/80 shadow-sm overflow-hidden">
        ...
        <div className="p-3 sm:p-4 border-b border-slate-200 flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center justify-between bg-slate-50/70">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs sm:text-sm bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)] focus:border-[var(--accent-color)]"
            />
          </div>
          ...
          <form onSubmit={handleBarcodeSubmit} className="flex items-center gap-1.5 flex-shrink-0">
            <div className="relative w-full sm:w-48">
              ...
              <input
                ...
                className="w-full pl-8 pr-2.5 py-2 text-xs bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)]"
              />
            </div>
              <button
                type="submit"
                className="px-3 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-2xl text-xs font-semibold shadow-sm"
              >
              Add
            </button>
          </form>
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
              selectedCategory === 'ALL'
                ? 'bg-[var(--accent-color)] text-white shadow-xs scale-[1.02]'
              ...
              <button
                ...
                className={`relative flex flex-col justify-between p-3.5 rounded-2xl border text-left transition-all duration-200 group ${
                  isOutOfStock
                    ...
                    : inCart
                     ? 'bg-[var(--accent-color)]/60 border-[var(--accent-color-light)] ring-2 ring-[var(--accent-color-light)] shadow-sm scale-[1.01]'
                     : 'bg-white border-slate-200/90 hover:border-[var(--accent-color-light)] hover:shadow-lg hover:-translate-y-1'
                }`}
              >
                {/* Top SKU & Badge */}
                   ...
                   <span
                     className={`text-[10px] font-bold px-2 py-0.5 rounded-full ring-1 ${
                       prod.stockQuantity <= prod.minStockThreshold
                         ? 'bg-red-50 text-red-700 border border-red-200 ring-red-300/60'
                ...
                <div className="my-2.5">
                  <h4 className="font-bold text-slate-900 text-xs sm:text-sm line-clamp-2 group-hover:text-[var(--accent-color)] transition-colors">
                    {prod.name}
                  </h4>
                ...
                <div className="flex items-center justify-between w-full pt-2.5 border-t border-slate-100">
                  ...
                  {inCart ? (
                    <span className="w-7 h-7 rounded-xl bg-[var(--accent-color)] text-white flex items-center justify-center text-xs font-black shadow-xs">
                      {inCart.quantity}
                    </span>
                  ) : (
                    <span className="w-7 h-7 rounded-xl bg-slate-100 group-hover:bg-[var(--accent-color)] group-hover:text-white text-slate-600 flex items-center justify-center transition-all">
      ...
      <div className="w-full lg:w-[420px] xl:w-[460px] flex flex-col bg-white rounded-3xl ring-1 ring-slate-200/80 shadow-sm overflow-hidden flex-shrink-0">
        {/* Cart Header */}
        <div className="p-4 border-b border-slate-200 bg-slate-50/80 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[var(--accent-color)] text-white flex items-center justify-center shadow-xs">
          ...
          <div className="flex items-center gap-1.5">
            {parkedCarts.length > 0 && (
                <button
                  ...
                  className="px-2.5 py-1 bg-amber-50 text-amber-800 border border-amber-200 rounded-2xl text-xs font-bold hover:bg-amber-100 flex items-center gap-1"
            ...
            {cartItems.length > 0 && (
              <>
                <button
                  ...
                  className="p-1.5 text-slate-600 hover:text-[var(--accent-color)] hover:bg-[var(--accent-color-light)] rounded-xl text-xs transition-colors"
        ...
        <div className="p-3 border-b border-slate-100 bg-white">
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="w-full text-xs font-medium text-slate-700 bg-slate-50 border border-slate-200 rounded-lg p-1.5 focus:outline-none focus:ring-1 focus:ring-[var(--accent-color)]"
            >
        ...
        <div className="p-3.5 border-t border-slate-200 bg-slate-50/70 space-y-3">
          ...
          <div className="space-y-1.5 text-xs text-slate-600">
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="flex items-center gap-1 text-slate-500">
                <Percent className="w-3 h-3 text-[var(--accent-color)]" />
                <span>Discount (%):</span>
              </span>
            ...
            <div className="flex justify-between items-baseline pt-2 border-t border-slate-200">
              ...
              <span className="font-extrabold text-xl text-[var(--accent-color-dark)]">${grandTotal.toFixed(2)}</span>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  onClick={() => setPaymentMethod(m.id)}
                   className={`flex flex-col items-center justify-center p-2 rounded-2xl text-xs font-semibold transition-all ${
                     isSelected
            ...
            <div className="space-y-2 bg-white p-2.5 rounded-lg border border-slate-200">
              <div className="flex items-center justify-between gap-2">
                ...
                <input
                  ...
                  className="w-24 px-2 py-1 text-right text-xs font-bold bg-slate-50 border border-slate-300 rounded focus:ring-1 focus:ring-[var(--accent-color)]"
                />
              </div>
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  className="flex-1 py-1 bg-[var(--accent-color-light)] hover:bg-[var(--accent-color-light)] text-[var(--accent-color-dark)] text-[10px] font-bold rounded transition-colors"
                >
                  Exact
                </button>
              </div>
          ...
          <button
            ...
            className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white rounded-2xl text-sm font-bold shadow-sm hover:shadow-lg transition-all flex items-center justify-center gap-2 cursor-pointer disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
      ...
      {isParkedModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-3xl ring-1 ring-slate-200/80 shadow-xl border border-slate-200 w-full max-w-md p-5 space-y-4">
              ...
              {parkedCarts.map(parked => (
                ...
                >
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-[var(--accent-color)] hover:bg-[var(--accent-color-dark)] text-white rounded-md text-xs font-semibold shadow-xs"
                  >
                    Resume
                  </button>
                </div>
            ...
            <div className="flex justify-end pt-2">
                 <button
                   ...
                   className="px-4 py-2 bg-slate-200 text-slate-700 rounded-2xl text-xs font-semibold"
                 >
                Close
              </button>
            </div>
