...
router.get('/', async (req, res) => {
    try {
        const returns = await Return_js_1.default.find().lean().sort({ date: -1 });
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!invoiceId || !items || items.length === 0) {
            return res.status(400).json({ success: false, error: 'Invoice ID and return items are required' });
        }
        // Find invoice
        const invoice = await Sale_js_1.default.findById(invoiceId);
        if (!invoice) {
        ...
        for (const retItem of items) {
            ...
            const product = await Product_js_1.default.findById(retItem.productId);
            if (!saleItem || !product)
                continue;
        ...
        const returnNumber = `RET-2026-${String(await Return_js_1.default.countDocuments({}) + 1).padStart(3, '0')}`;
        const newReturn = {
            returnNumber,
            invoiceId,
        ...
        const ret = new Return_js_1.default(newReturn);
        await ret.save();
