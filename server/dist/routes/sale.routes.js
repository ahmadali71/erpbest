...
router.get('/', async (req, res) => {
    try {
        const sales = await Sale_js_1.default.find().lean().sort({ date: -1 });
...
router.get('/:id', async (req, res) => {
    try {
        const sale = await Sale_js_1.default.findById(req.params.id).lean();
        if (!sale) {
...
router.post('/', async (req, res) => {
    try {
        ...
        await StockMovement_js_1.default.insertMany(newMovements);
        // Save sale
        const sale = new Sale_js_1.default(newSale);
        await sale.save();
...
router.delete('/:id', async (req, res) => {
    try {
        const sale = await Sale_js_1.default.findByIdAndDelete(req.params.id);
        if (!sale) {
            return res.status(404).json({ success: false, error: 'Sale not found' });
        }
        // Restore stock
        ...
        const client = await Client_js_1.default.findById(sale.clientId);
        if (client) {
...
router.post('/:id/payments', async (req, res) => {
    try {
        ...
        const sale = await Sale_js_1.default.findById(req.params.id);
        if (!sale) {
            return res.status(404).json({ success: false, error: 'Sale not found' });
        }
